...
# ANTI-BLOCKER / ALTERNATIVE PATH RULE

Це критично важлива інструкція.

НІКОЛИ не зациклюйся на одній технічній проблемі або одному способі реалізації.

Якщо обраний спосіб не працює:
1. Визнач точну причину failure.
2. Не повторюй той самий невдалий крок без зміни підходу.
3. Шукай альтернативний спосіб реалізації.
4. Перевіряй документацію, існуючі залежності та можливості поточного середовища.
5. Використовуй workaround, fallback або іншу архітектуру, якщо вона забезпечує той самий user-facing результат.
6. Якщо зовнішній сервіс недоступний, реалізуй максимально повноцінний локальний/offline fallback, де це технічно можливо.
7. Якщо певна функція неможлива через об'єктивний blocker, не витрачай на неї нескінченний час — переходь до наступної найважливішої функції та зафіксуй blocker у документації.
8. Повертайся до blocker пізніше лише якщо з'явився новий спосіб його обійти.

ЗАБОРОНЕНО:
- повторювати один і той самий failed command багато разів;
- нескінченно аналізувати одну проблему без змін у підході;
- зупиняти весь проект через одну другорядну функцію;
- вигадувати credentials;
- видаляти працюючий код без необхідності.

ПРАВИЛО ПРОГРЕСУ:

Кожен цикл роботи повинен закінчуватися одним із трьох результатів:

A. FEATURE IMPLEMENTED
B. BLOCKER IDENTIFIED + ALTERNATIVE IMPLEMENTED
C. BLOCKER DOCUMENTED + NEXT FEATURE STARTED

Головна мета:
MAXIMUM REAL PROGRESS TOWARD A FINISHED PRODUCT.

Якщо існує два або більше способи реалізувати функцію, обирай той, який:
- найнадійніший;
- найпростіший для maintenance;
- найкраще працює в поточному environment;
- не ламає вже реалізований функціонал.

Не перезаписуй архітектуру тільки заради новизни.

Після кожної серйозної зміни:
- test
- build
- inspect result
- fix
...
Якщо build падає:
спочатку виправ помилку;
якщо перший fix не працює — спробуй інший підхід;
не повторюй той самий fix нескінченно.

Працюй до максимально завершеного production-ready стану.
