...
const Analytics = () => {
  ...
  const Chart = ({ title, data, color }: { title: string; data: number[]; color: string }) => {
    ...
    <div className="min-h-screen bg-slate-50 p-6 max-w-4xl mx-auto">
      ...
      {matrices.length === 0 ? (
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm">
          <Chart title="Claridad" data={matrices.map(m => m.clarityIndex)} color="bg-green-500" />
          <Chart title="Intensidad del Bucle" data={matrices.map(m => m.loopIntensity)} color="bg-red-500" />
          <Chart title="Fricción en Pareja" data={matrices.map(m => m.coupleFriction)} color="bg-orange-500" />
          <Chart title="Riesgo de Insomnio" data={matrices.map(m => m.sleepLatencyRisk)} color="bg-purple-500" />
        </div>
      )}
    </div>
  );
