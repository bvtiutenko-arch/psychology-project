...
const ConnectionMap = () => {
  ...
  const Node = ({ label, value, color }: { label: string; value: string; color: string }) => (
    <div className="flex flex-col items-center mb-8 relative">
      ...
      <div className="absolute -bottom-8 left-1/2 transform -translate-x-1/2 w-0.5 h-8 bg-slate-300"></div>
    </div>
  );
    ...
    <div className="min-h-screen bg-slate-50 p-6 max-w-md mx-auto">
      ...
      {!matrix ? (
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm flex flex-col items-center">
          <Node label="Disparador" value={matrix.triggerEvent} color="bg-blue-500" />
          <Node label="Pensamiento" value={matrix.cognitiveBias} color="bg-purple-500" />
          <Node label="Comportamiento" value={matrix.somaticCompulsion} color="bg-orange-500" />
          <Node label="Consecuencia" value={matrix.feedbackLoop} color="bg-red-500" />
