  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      try {
        ...
        console.error('Error saving causal matrix offline:', offlineError);
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-6 max-w-md mx-auto">
      <header className="flex items-center mb-8">
        ...
        <h1 className="text-xl font-bold ml-2 text-slate-800">Registrar Evento</h1>
      </header>
        ...
        {latestMetrics ? (
          ...
          <>
            ...
            <form onSubmit={(e) => e.preventDefault()}> {/* Prevent default form submission */}
              <SelectField label="1. Herida Raíz" value={formData.rootWound} onChange={(e) => handleChange('rootWound', e.target.value)} enumObject={RootWound} />
              <SelectField label="2. Evento Disparador" value={formData.triggerEvent} onChange={(e) => handleChange('triggerEvent', e.target.value)} enumObject={TriggerEvent} />
              ...
              <button
                ...
                className="w-full mt-6 bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-4 rounded-lg transition-colors disabled:bg-blue-300 disabled:cursor-not-allowed"
              >
                Revisar Matriz
              </button>
            </form>
